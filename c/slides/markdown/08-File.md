...
* Contengono solo caratteri ASCII stampabili e alcuni caratteri di controllo (e.g., *newline* e *tabulazione*)
...
* Un file può essere aperto per diversi scopi: lettura, scrittura e append (scrittura alla fine del file). La stringa *mode* può quindi valere:
  ...
  * **a+** (lettura e append) : se il file non esiste, un nuovo file viene creato; l'accesso in lettura avviene dall'inizio, mentre la scrittura è sempre effettuata alla fine del file
...
* *fopen()* restituisce:
  * 0 se la chiusura avviene con successo
  * 1 in caso di errore
...
* Le funzioni *fread()* ed *fwrite()* vengono utilizzate per leggere e scrivere dati in formato binario e richiedono i seguenti parametri:
  * *pt* Puntatore all'area di memoria che contiene di dati scrivere o in cui memorizzare i dati letti
  * *size* La dimensione del singolo elemento letto o scritto
  * *nelem* Il numero degli elementi letti o scritti
